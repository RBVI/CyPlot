...
public class JSUtils {
	 ...
	 * @param editor  the boolean which determines whether the graph will utilize
	 *                the plotly graph editor
	 */
	...
	/**
	 * Generate the string necessary to generate the base plotly graph editor.
	 *
	 * @param data the String representation of user-selected column data from
	 *             cytoscape
	...
	/**
	 * Generate the string necessary to load a js file into the selected plot as a
	 * seperate script within the HTML.
	 *
	 * @param builder the StringBuilder we'll write the javascript into
	 ...
	 * path can be customized by altering "user.home" to suit individual needs.
	 * Useful for opening the code in external browsers for debugging purposes, but
	 * not essential to the functionality of the app.
	 *
	 ...
	 * @param mode            the String which determines the form the data will
	 *                        take ("markers" for scatter or "lines" for line)
	 * @param selectionString a cytoscape command callback for selection
	 * @param nameSelection   the cytoscape data column we use to show the names of
	 *                        points
	 * @param nameArray       the list of node names
	 * @param title           the plot title
	 * @param xLabel          the label for the x-axis
	 * @param yLabel          the label for the y-axis
	 * @param editor          the boolean that controls whether the graph will open
	 *                        in the plotly graph editor
...
//			Map<String, String> nameTraceMap, String selectionString, String nameSelection, String title, String xLabel,
//			String yLabel, String dataExtra, String layoutExtra, String mode, boolean editor, boolean yValLog,
//			boolean xValLog) {
...
//	}
	...
	public static String getFilledAreaPlot(String x, String y, String mode, String selectionString,
			String nameSelection, String nameArray, String title, String xLabel, String yLabel, boolean editor) {
		StringBuilder builder = new StringBuilder();
		getPreamble(builder, editor, title);
		if (!editor) {
			builder.append("<body><div id=\"CyPlot\" style=\"width: 100%; height: 100%\"></div>\n");
			builder.append("<script> var trace1 = { x: " + x + ", y: " + y
					+ ", fill: 'tonexty', type: 'scatter', name: 'trace', mode: '" + mode + "', text: " + nameArray
					+ "};\n");
			...
			builder.append("<body>");
			builder.append("<div id=\"CyPlot\" style=\"width: 100%; height: 100%\"></div>\n");
			// builder.append("<a style=\"position:absolute; left: 100px; top: 30\"
			// onclick='hideControls()'>Hide Controls</a>\n");
			builder.append("<script type=\"text/javascript\" >\n");
			builder.append("var dataSources = {'" + xLabel + "': " + x + ", '" + yLabel + "': " + y + "};\n");
			builder.append("var trace1 = { x: " + x + ", y: " + y
					+ ", fill: 'tonexty', type: 'scatter', name: 'trace', mode: '" + mode + "', text: " + nameArray
					+ "};\n");
			builder.append("var data = [trace1];\n");
			builder.append(getLabelCode(xLabel, yLabel, title, false));
			builder.append(
					"ReactDOM.render(React.createElement(app.App.default, { dataSources: dataSources, data: data, layout: layout }), document.getElementById('CyPlot'));\n");
			builder.append("</script>\n");
			if (nameSelection != null && nameArray != null) {
	 ...
	 */
	public static String getVolcanoPlot(String x, String y, String selectionString, String nameSelection,
		...
		if (!editor) {
			...
			builder.append("<script>\n");
			builder.append("var trace1 = { x: " + x + ", y: " + y
					+ ", type: 'scatter', mode: 'markers', name: 'trace', text: " + nameArray + "};\n");
			builder.append("var data = [trace1];\n");
	...
	/**
	 * Generate the string necessary to generate a bar chart in plotly, either with
	 * or without the plotly graph editor.
	 *
	 * @param x               the user-selected x-column data
	 * @param y               the user-selected y-column data
	 * @param selectionString a cytoscape command callback for selection
	...
	public static String getBarChart(String x, String y, String selectionString, String nameSelection, String nameArray,
			String title, String xLabel, String yLabel, boolean editor, boolean yValLog) {
		StringBuilder builder = new StringBuilder();
		getPreamble(builder, editor, title);
		if (!editor) {
			builder.append("<body><div id=\"CyPlot\" style=\"width: 100%; height: 100%\"></div>\n");
			...
			if (yValLog) {
				if (title == null)
					title = yLabel + " of " + xLabel;

				builder.append("var layout = {showlegend: " + false
						+ ", legend: { x: 1, y: 0.5 }, hovermode: 'closest', xaxis: { title:'" + xLabel
						+ "',type: 'log', automargin: true}, yaxis: { title:'" + yLabel
						+ "', automargin: true}, title: '" + title + "'};\n");
			} else {
				builder.append(getLabelCode(xLabel, yLabel, title, false));
			}
...
//			builder.append(getLabelCode(xLabel, yLabel, title, false));
			...
			// builder.append("var config = {'responsive':true};\n"); // Will this work? Can
			// we pass config using the editor?
			builder.append(
					"ReactDOM.render(React.createElement(app.App.default, { dataSources: dataSources, data: data, layout: layout }), document.getElementById('CyPlot'));\n");
	...
	// getHeatMap javadoc by Liam, don't actually know if this is accurate (Lilly
	// can you check?)
	/**
	 * Generate the string necessary to generate a heat map in plotly, either with
	 * or without the plotly graph editor.
	 *
	 * String html = JSUtils.getHeatMap(rowHeaders, columnHeaders, colData,
	 * colorPalette, xLabel, yLabel, title, editor);
	 * 
	 * @param rowHeaders      the list of row headers. Also defines the row order.
	 * @param columnHeaders   the list of column headers. Also defines the column
	 *                        order.
	 * @param colData         a map of the data, organized by the column name as the
	 *                        key and a JSON-formatted array with the column data
	 ...
	 * @param colorPalette    a color palette. Could be either a String, in which
	 *                        case it's assumed to be a Plotly palette, or a
	 *                        Palette, and we'll provide RGB values.
	 ...
	 */
	public static String getHeatMap(List<String> rowHeaders, List<String> columnHeaders, Map<String, List<?>> colData,
			String selectionString, String nameSelection, Object colorPalette, String xLabel, String yLabel,
		...
		if (editor) {
			...
			builder.append(
					"{ dataSources: dataSources, data: data, layout: layout }), document.getElementById('CyPlot'));\n");
		} else {
			builder.append("var config = {'responsive':true};\n");
			builder.append("Plotly.newPlot('CyPlot', data, layout, config);");
			getResizeCode(builder);
	...
	/**
	 * Generate the string necessary to generate a violin plot in plotly, either
	 * with or without the plotly graph editor.
	 *
	 * @param x         the user-selected x-column data
	 * @param y         the user-selected y-column data
	 ...
	 * @param xLabel    the label for the x-axis
	 * @param yLabel    the label for the y-axis
	 * @param editor    the boolean that controls whether the graph will open in the
	 *                  plotly graph editor
	 * @return the assembled violin plot code
	 */
	public static String getViolinPlot(Map<String, String> traceMap, String selectionString, String nameSelection,
			Map<String, String> traceNamesMap, List<String> traceOrder, String title, String xlabel, String ylabel,
			String dataExtra, String layoutExtra, boolean editor) {
		...
		/*
		 * builder.
		 * append("var layout = { hovermode: 'closest', showlegend: true, title: '"
		 * +title+"',");
		 * builder.append("xaxis: { title:'"+xlabel+"'}, yaxis: {title:'"+ylabel+"'},");
		 * if (layoutExtra) builder.append(layoutExtra); builder.
		 * append("legend: { tracegroupgap: 0 }, violingap: 0, violingroupgap: 0, violinmode: 'overlay' };\n"
		 * );
		 */
	...
	/**
	 * Generate the string necessary to generate a dot plot or in plotly, either
	 * with or without the plotly graph editor.
	 *
	 * @param x         the user-selected x-column data
	 * @param y         the user-selected y-column data
	...
	public static void getClickCode(StringBuilder builder, String plot, String selectionString, String nameSelection,
			boolean isEditor) {
	...
	/**
	 * Generate the string necessary to support integrating the plotly click
	 * selection with Cytoscape. This is not in an independent javascript file
	 * because we need to construct the script from our current java objects.
	 *
	 * @param builder         the StringBuilder we'll write the javascript into
	 ...
	 *                        PlotlyEditor. This means we have to find the
	 *                        appropriate div by searching for a particular CSS
	 *                        class.
	 */
	public static void getClickCode(StringBuilder builder, String plot, String selectionString, String nameSelection,
			String dataElement, boolean isEditor) {
	...
	public static void getLassoCode(StringBuilder builder, String plot, String selectionString, String nameSelection,
			boolean isEditor) {
	...
	/**
	 * Generate the string necessary to support integrating the plotly Lasso
	 * selection with Cytoscape. This is not in an independent javascript file
	 * because we need to construct the script from our current java objects.
	 *
	 * @param builder       the StringBuilder we'll write the javascript into
	 ...
	 * @param dataElement   the data element to use for the selection name
	 * @param isEditor      if <b>true</b> we'll be integrating this into the
	 *                      PlotlyEditor. This means we have to find the appropriate
	 *                      div by searching for a particular CSS class.
	 */
	public static void getLassoCode(StringBuilder builder, String plot, String selectionString, String nameSelection,
			String dataElement, boolean isEditor) {
		if (dataElement == null)
		...
		if (isEditor) {
			...
			if (selectionString == null)
				builder.append(
						"            nodelist+= ('," + nameSelection + ":' +data.points[i]." + dataElement + ");\n");
			else
	...
	/**
	 * FIXME: Note that since this overwrites layout, we probably don't want to do
	 * it this way. I think we should assemble the layout object more incrementally.
	 * Perhaps we should create a PlotlyLayout class so we can add features to it
	 * over time? The ".toString()" method of the class would return the constructed
	 * layout string.
	 *
	 * This method creates the layout object that we'll pass to plotly. It adds
	 * support for title, axes and legend.
	 *
	 ...
	 *
	 */
	public static String getLayoutCode(String xLabel, String yLabel, String title, String layoutExtra,
			boolean showLegend, boolean xValLog, boolean yValLog) {
	...
	public static String getLabelCode(String xLabel, String yLabel, String title, boolean showLegend) {
		if (title == null)
			title = xLabel + " vs " + yLabel;
		return "var layout = {showlegend: " + showLegend
				+ ", legend: { x: 1, y: 0.5 }, hovermode: 'closest', xaxis: { title:'" + xLabel
				+ "', automargin: true}, yaxis: { title:'" + yLabel + "', automargin: true}, title: '" + title + "'};";
	}
	...
	/**
	 * Generate the string necessary to support integrating the plotly graph
	 * resizing function. This is not in an independent javascript file because we
	 * need to construct the script from our current java objects.
	 *
	 * @param builder the StringBuilder we'll write the javascript into
	 */
	...
	/**
	 * Load the code that adds a link to hide and show the controls panel in the
	 * editor view.
	 *
	...
	// Function to get the string to produce regression line
	public static String getRegressionLine() {
		String regrStr = "var";
	...
	public static String getXYPlot(String type,
            ...
            String scaleLabel, boolean editor,boolean xValLog,boolean yValLog,boolean regr,double slope,double intercept) {
			StringBuilder builder = new StringBuilder();
			...
			String dataSources = "var dataSources = {";
			int traceNumber = 1;
			System.out.println(xTraceMap.get("trace"));
//			var dataExtra2 = "var regressionLine = " + xTraceMap.get("trace") + ".map(x => x + 3);\n";
//			builder.append(dataExtra2);
...
//			builder.append("var traceRegressionLine = {x:" + xTraceMap.get("trace")
//	        + ",y: regressionLine,mode: 'lines',type: 'scatter',name: 'Linear Regression'};\n");
			
//			traceStr+=dataExtra3; 
			if(regr) {
			builder.append(traceStr.substring(0, traceStr.length()-1)+",traceRegressionLine];\n");
			}else {
				builder.append(traceStr.substring(0, traceStr.length()-1)+"];\n");
			}
	...
	/**
	 * This method loads data from the resources in the CyPlot jar file. While the
	 * title is loadJS, we also use it for css and other files we don't want to
	 * inject directly via strings.
	 *
	 * @param builder This is the {@link StringBuilder} that we use to write the
	 *                embedded content into.
	 * @param js      The name of the file, including the prefix that we're going to
	 *                look for in the resources
	 */
	private static void loadJS(StringBuilder builder, String js) {
		URL plotly = JSUtils.class.getClassLoader().getResource(js);
		try (Stream<String> stream = new BufferedReader(
				new InputStreamReader(plotly.openConnection().getInputStream(), StandardCharsets.UTF_8)).lines()) {
			stream.forEach((s) -> {
				builder.append(s + "\n");
			});
		} catch (Exception e) {
	...
	public static String getHistogramPlot(String x, String y, boolean yValLog, String selectionString,
			String nameSelection, String nameArray, String title, String xLabel, String yLabel, boolean editor) {
		StringBuilder builder = new StringBuilder();
		getPreamble(builder, editor, title);
		if (!editor) {
			...
			if (yValLog) {
				...
				builder.append("var layout = {showlegend: " + false
						+ ", legend: { x: 1, y: 0.5 }, hovermode: 'closest', xaxis: { title:'" + xLabel
						+ "', automargin: true}, yaxis: { title:'" + yLabel
						+ "',type: 'log', automargin: true}, title: '" + title + "'};\n");
			} else {
